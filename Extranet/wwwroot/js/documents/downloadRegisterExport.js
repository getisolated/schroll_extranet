$(() => {
    ...
    $('body').on('click', '.btnDownloadRegisterExport', function (e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $('body').on('click', '.btnDownloadRegisterExportLine', function (e) {
        e.preventDefault();
        e.stopPropagation();
        var ids = ["Registre"];
        var entryNo = $(this).attr('data-docno');
...
function downloadDocsLine(ids, entryNo) {
    var docNos = ids;
    //const searchParams = new URLSearchParams(window.location.search);
    //var from = searchParams.get('from');
    //var to = searchParams.get('to');
    ...
    $.ajax({
        url: '/Registre/DownloadDocsChrono?entryNo=' + entryNo,
        type: 'POST',
        contentType: 'application/json',
        data: "{}",
        success: function (data) {
            ...
            if (stopTimer) {
                ...
                downloadTimer = setTimeout(function () { downloadDocsLine(docNos, entryNo) }, 10000);
            }
        },
        error: function (xhr, status, error) {
            clearTimeout(downloadTimer);
            $('.downloadLoader').each(function () {
                $(this).css('display', 'none');
                //TODO : afficher un message d'erreur à la place des href ?
            });
        }
    });
}